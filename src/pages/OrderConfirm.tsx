  ...
  return (
    <Flex
      ...
      <Box width="100%" maxWidth="1240px" mx="auto" paddingTop="8rem">
        ...
        <Box width="100%" p={6} border={`1px solid ${theme.colors.main}`} borderRadius="3xl">
            ...
            <Box key={item.id} width="100%" p={10} mb={3}>
              ...
              >
                ...
                <Box flex="1" textAlign={['center', 'center', 'left']}>
                  ...
                  <Text fontSize="1.2rem">최대 인원: {item.max_guest}인</Text>
                  <Divider my={6} borderColor={`${theme.colors.main}`} />
                  <Text fontSize="1.2rem">
          ...
          <Box width="100%" p={10} mt={20}>
            <Flex justifyContent="right" width="100%" fontSize="2.4rem" fontWeight="700">
              <Text>결제금액</Text>
              <Text ml={6}>{totalAmount.toLocaleString('ko-KR', { style: 'currency', currency: 'KRW' })}</Text>
            </Flex>
          </Box>
        </Box>
      </Box>
